...
class ViT(pl.LightningModule):
    def __init__(self, num_classes,epochs,model_name=None, config=None, lr=1e-4):
        super().__init__()
        ...
        if(config=="online"):
            ...
            self.model = ViT.load_from_checkpoint(model_name).model
        elif(config!=None):
...
if __name__=="__main__":
    ...
    if(args.start=="scratch"):
        model=ViT(2,args.epochs,config=config)
    elif(args.start=="local" or args.start=="online"):
        model=ViT(2,args.epochs,config=args.start,model_name=args.import_from)
    else:
        model=ViT.load_from_checkpoint(args.start,epochs=args.epochs,num_classes=2)
