...
def load_checkpoint(model, checkpoint_path, device):
    ...
    print(
        f"Loaded {len(compatible)} tensor(s) from checkpoint: {checkpoint_path}"
    )
    if skipped:
...
def resolve_default_paths():
    ...
    return {
        ...
        "output": cnn_dir / "adience_folds_finetuned.pth",
    }
...
def main():
    ...
    print(f"\n{'=' * 50}")
    print("PHASE 1: Freeze backbone, train FC head only")
    ...
    print(f"\nDone. Best val acc: {best_val_acc:.4f}. Saved to: {output_path}")
