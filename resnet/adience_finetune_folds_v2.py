...
def resolve_default_paths(output_name):
    cnn_dir = Path(__file__).resolve().parent
    gender_dir = cnn_dir.parent
    checkpoint_dir = gender_dir / "Checkpoint"
    return {
        "gender_dir": gender_dir,
        "img_dir": gender_dir / "age-gender-data" / "aligned",
        "label_dir": gender_dir / "label txt",
        "checkpoint": cnn_dir / "celeba_scratch.pth",
        "output": checkpoint_dir / output_name,
    }


def build_arg_parser(defaults):
    parser = argparse.ArgumentParser()
    parser.add_argument("--data_dir", type=str, default=str(defaults["gender_dir"]))
...
def run_training(model_builder, output_name, head_name):
    defaults = resolve_default_paths(output_name)
    ...
    img_dir = Path(args.img_dir).resolve() if args.img_dir else base_dir / "age-gender-data" / "aligned"
    label_dir = Path(args.label_dir).resolve() if args.label_dir else base_dir / "label txt"
    ...
    print("Loading val fold (same as held-out test fold)...")
...
def main():
    run_training(
        ...
        output_name="gender-cnn-simple-best.pth",
        head_name="Linear",
    )
